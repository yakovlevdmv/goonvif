...
import (
	...
	media "github.com/use-go/goonvif/Media"
...
func getSearchStructByName(name string) (interface{}, error) {
	switch name {
	case "FindEvents":
		return &search.FindEvents{}, nil
	case "GetEventSearchResults":
		return &search.GetEventSearchResults{}, nil
	case "FindRecordings":
		return &search.FindRecordings{}, nil
	default:
		return nil, errors.New("there is no such method in the Search service")
	}
}

func getRecordingStructByName(name string) (interface{}, error) {
	switch name {
	case "GetRecordings":
